...
<div class="modal fade" id="modalAttribute">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title fw-bold">Kelola Atribut Kustom</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            
            <div class="modal-body">
                <form action="{{ route('w.attribute.store', $warehouse->id) }}" method="POST" class="mb-4">
                    @csrf
                    <label class="small text-muted mb-1">Tambah Kolom Baru</label>
                    <div class="input-group">
                        <input type="text" name="attribute_name" class="form-control" placeholder="Contoh: Warna, Size, Merk" required>
                        <button class="btn btn-primary" type="submit">Simpan</button>
                    </div>
                </form>
                ...
                <label class="small text-muted mb-2">Daftar Atribut Saat Ini:</label>
                <div class="list-group">
                    @forelse($attributes as $attr)
                    <div class="list-group-item d-flex justify-content-between align-items-center">
                        <span>{{ $attr->attribute_name }}</span>
                        
                        <form action="{{ route('w.attribute.delete', [$warehouse->id, $attr->id]) }}" method="POST">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn btn-sm btn-outline-danger border-0" onclick="return confirm('Yakin hapus atribut ini? Data yang sudah tersimpan di barang tidak akan hilang, tapi kolomnya akan disembunyikan.')">
                                <i class="bi bi-trash"></i> Hapus
                            </button>
                        </form>
                    </div>
                    @empty
                    <div class="text-center text-muted py-2">Belum ada kolom kustom.</div>
                    @endforelse
                </div>
            </div>
        </div>
